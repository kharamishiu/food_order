import { CartCustomization, CartStorage } from "@/type";
...
export const useCartStore = create<CartStorage>((set, get) => ({
    items: [],
    ...
    addItem: (item) => {
        ...
        if (exist) {
            set({
                items: get().items.map((i) => i.id === item.id && areCustomizationsEqual(
                    i.customizations ?? [], customizations) ? { ...i, quantity: i.quantity + 1 } : i)
            });
        } else {
            set({
    ...
    getTotalPrice: () =>
        get().items.reduce((total, item) => {
            ...
            const customPrice = item.customizations?.reduce(
                (s: number, c: CartCustomization) => s + c.price, 0) ?? 0;
